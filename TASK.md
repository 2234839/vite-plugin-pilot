...
/loop 先检查 TASK.md 中是否有未完成的任务请逐项完成并在充分test验证再继续下一项，如果没有则请请完善当前项目：然后想办法完善项目，目的是让此项目对于agent而言用起来得心应手，请直接执行，不要询问我，不要进入计划模式,修改完毕后需要进行实际运行测试，请自我完善，不要询问我任何事情，也不要切换其他模式（例如 plan mode），我已经在浏览器打开了playground页面（咱们基于他进行测试和改进，但是不要特化成只能处理他）需要测试 vue react 原生js这几种情况，所以任何改动都需要你使用自身进行实际测试，确保pilot非常好用而且没有任何问题
使用咱们插件的人 肯定要频繁重启 dev server的，所以不用考虑太多的旧客户端的向后兼容
修改代码后记得同步更新对应的文档
完成每一轮后需要更新测试用例的就更新测试用例（scripts/test.ts与tests.md），然后执行 tests.md 中的所有测试用例确保无问题回归
每一轮改进完毕后立刻开始下一轮

## TASKS
[x] 当前各项功能都没有支持shadow DOM ，例如 web component 的组件，请支持
[x] 用户使用含 alt 的组合键时不应触发element-inspector,只有单独使用时应该触发
[x]exec 中如果有操作元素，那么快照是否根据元素的位置去将上下文返回是否更好，快照返回文本有上限我记得，那么就可以根据这个来确定需要被返回的文本片段起始和结束位置（尽可能将被操作元素及其上下文包含在内）
